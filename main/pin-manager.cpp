...
PinManager::PinManager( I2CBUSPtr i2c, QueuePtr interruptQueue ) : mInterruptQueue( interruptQueue ), mI2C( i2c ) {
    mPinManagerMCP = PinMcpManagerPtr( new PinMcpManager( i2c, I2C_ADDR_MCP, interruptQueue ) );
}
...
PinManager::createPin( uint8_t pinType, uint8_t pinReference, uint8_t direction, uint8_t pulldown, uint8_t pullup ) {
    ...
    switch( pinType ) {
        case PIN_TYPE_ESP32:
            pin = PinPtr( new PinESP32( (gpio_num_t)pinReference, direction, pulldown, pullup ) ); 
            break;
        case PIN_TYPE_MCP:
